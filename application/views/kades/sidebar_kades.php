<!-- Sidebar -->
<ul class="navbar-nav sidebar sidebar-dark accordion"
			style="background: -webkit-linear-gradient(top, #2dfbff 0%, #000080 100%);" id="accordionSidebar">

			<!-- Sidebar - Brand -->
			<a class="sidebar-brand d-flex align-items-center justify-content-center" href="index.html">
                <div class="">
                    <img src="<?= base_url('assets/dashboard/') ?>img/logo_SIRA.png" width="90" />
                </div>
                <div class="sidebar-brand-text mx-0">KEPALA DESA</div>
            </a>

			<!-- Divider -->
			<hr class="sidebar-divider my-0">

			<!-- Nav Item - Dashboard -->
			<li class="nav-item active">
				<a class="nav-link" href="dashboard_kades.html">
					<i class="fas fa-fw fa-tachometer-alt"></i>
					<span>Dashboard</span></a>
			</li>

			<!-- Divider -->
			<hr class="sidebar-divider">

			<!-- Heading -->
			<div class="sidebar-heading">
				MENU
			</div>

			<!-- Nav Item - Main Menu -->

			<li class="nav-item">
                <a class="nav-link" href="list_admin_kades.php">
                    <i class="fa fa-users"></i>
                    <span>Data Admin</span>
                </a>
            </li>

			<li class="nav-item">
                <a class="nav-link" href="list_data_ketua_rt_kades.html">
                    <i class="fa fa-users"></i>
                    <span>Data ketua RT</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="list_data_warga_kades.html">
                    <i class="fa fa-book"></i>
                    <span>Data Warga</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="lap_permohonan_kades.html">
                    <i class="fa fa-book"></i>
                    <span>Lap. Permohonan</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="arsip_surat_masuk_kades.html">
                    <i class="fa fa-book"></i>
                    <span>Arsip Surat Masuk</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="arsip_surat_keluar_kades.html">
                    <i class="fa fa-book"></i>
                    <span>Arsip Surat Keluar</span>
                </a>
            </li>




			<!-- Divider -->
			<hr class="sidebar-divider">

			<!-- Heading -->
		

			<!-- Nav Item - Menu Laporan -->
		

		

			<!-- Sidebar Toggler (Sidebar) -->
			<div class="text-center d-none d-md-inline">
				<button class="rounded-circle border-0" id="sidebarToggle"></button>
			</div>

		</ul>
		<!-- End of Sidebar -->